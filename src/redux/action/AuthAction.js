          ...
          .then((res) => {
            if (res.data.status) {
              ...
              dispatch({
                ...
                payload: { email, username, userid: res.data.id },
              });
              ...
              dispatch({
                type: USER_REGISTER_FAILED,
                payload: { isEmail: false, isUsername: true },
              });
            }
          })
          .catch((err) => {
...
export const Verify = (props) => {
  return (dispatch) => {
    ...
    Axios.get(`${API_URL}/users/verified`, {
      ...
      .then((res) => {
        //kalo token masih aktif
        console.log(res.data);
        dispatch({ type: VERIFY_SUCCESS, payload: res.data });
      })
      .catch((err) => {
        //kalo token udah expired (status 401 ada di auth helper)
        console.log(err);
  ...
  return (dispatch) => {
    ...
    if (username === "" || password === "") {
      dispatch({
        ...
        payload: "Please fill username and password.",
      });
    } else {
...
export const SendEmailPassword = (email) => {
  return (dispatch) => {
    ...
    if (email === "") {
      dispatch({
        type: SEND_EMAIL_PASSWORD_FAILED,
    ...
    if (password !== confirmpassword) {
      ...
      Axios.get(`${API_URL}/users/resetpassword`, databody)
        ...
        .catch((err) => {
          ...
          dispatch({
            type: RESET_PASSWORD_FAILED,
            payload: { isResetLinkExpired: true },
          });
        });
    }
  };
